...
class BboxLoss(nn.Module):
    ...
    def __init__(self, reg_max: int = 16, iou_type: str = "siou"):
        ...
        Args:
            reg_max (int): Maximum regularization value for DFL loss.
            iou_type (str): bounding-box regression loss type, "siou" or "ciou" (default siou).
        """
        super().__init__()
    ...
    def forward(
        ...
        # bbox_iou keyword args are capitalized: SIoU/CIoU
        iou_kwargs = {"SIoU": True} if self.iou_type == "siou" else {"CIoU": True}
        iou = bbox_iou(pred_bboxes[fg_mask], target_bboxes[fg_mask], xywh=False, **iou_kwargs)
...
class v8DetectionLoss:
    ...
    def __init__(
        ...
        self.bbox_loss = BboxLoss(m.reg_max, iou_type=os.getenv("ULTRALYTICS_IOU_LOSS", "siou")).to(device)
