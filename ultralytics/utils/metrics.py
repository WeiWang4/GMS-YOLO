...
def bbox_iou(
    ...
    Args:
        ...
        SIoU (bool, optional): If True, calculate SCYLLA IoU (SIoU Loss https://arxiv.org/pdf/2205.12740.pdf).
        eps (float, optional): A small value to avoid division by zero.
    ...
    if CIoU or DIoU or GIoU or SIoU:
        ...
        if SIoU:  # SIoU Loss https://arxiv.org/pdf/2205.12740.pdf
            ...
            omiga_w, omiga_h = (
                torch.abs(w1 - w2) / torch.max(w1, w2),
                torch.abs(h1 - h2) / torch.max(h1, h2),
            )
