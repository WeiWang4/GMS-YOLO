...
def parse_model(d, ch, verbose=True):
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
            ...
            if m is MSECA:  # MSECA is an identity attention: output channels = input channels
                c2 = c1
            ...
            if m is C2fCIB:
                legacy = False
        elif m is GAM:  # GAM is an identity attention: output channels = input channels; args are (ratio, kernel_size)
            c1 = ch[f]
            c2 = c1
            args = [c1, *args]
        elif m is AIFI:
